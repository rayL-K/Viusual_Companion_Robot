...
class ConfigLoadingTest(unittest.TestCase):
    ...
    def test_default_config_loads_live2d_paths(self) -> None:
        ...
        self.assertEqual(config.backend.llm, "cloud")
        self.assertEqual(config.backend.vision, "local")
        self.assertEqual(config.backend.asr, "local")
        self.assertEqual(config.backend.tts, "local")
        self.assertEqual(config.backend.vad, "local")
        self.assertEqual(config.model_paths.pose, "models/pose/yolov8n-pose.rknn")
        self.assertEqual(config.model_paths.active_speaker, "models/active_speaker/light-asd-ava.onnx")
...
"""
    ...
    def test_config_rejects_cloud_vision_fallback(self) -> None:
        config_text = """
app: {name: visual_companion_robot, mode: test}
runtime: {display: ':0', log_level: INFO}
backend: {llm: cloud, vision: cloud, asr: local, tts: local, vad: local}
modules:
  live2d_display:
    enabled: true
    model_name: Strawberry_Rabbit
...
"""
        with tempfile.TemporaryDirectory() as temp_dir:
            config_path = Path(temp_dir) / "app.yaml"
            config_path.write_text(config_text, encoding="utf-8")
            with self.assertRaisesRegex(ConfigError, "必须为 local"):
                load_app_config(config_path)
