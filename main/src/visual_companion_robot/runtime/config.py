...
class BackendConfig:
    ...
    def from_mapping(cls, data: Mapping[str, str]) -> "BackendConfig":
        def _valid(key: str, fallback: str) -> str:
            ...
            if key in cls.LOCAL_ONLY and value != "local":
                raise ConfigError(f"后端 `{key}` 必须为 local，项目不允许云端或客户端降级")
            return value
        return cls(
            llm=_valid("llm", "cloud"),
            vision=_valid("vision", "local"),
...
class ModelPaths:
    ...
    yolo: str = "models/yolo/yolov5s-640-640.rknn"
    pose: str = "models/pose/yolov8n-pose.rknn"
    yunet: str = "models/face/yunet.onnx"
    sface: str = "models/face/sface.onnx"
    emotion: str = "models/emotion/emotion-ferplus-8.onnx"
    active_speaker: str = "models/active_speaker/light-asd-ava.onnx"
    ...
    def from_mapping(cls, data: Mapping[str, str]) -> "ModelPaths":
        return cls(
            ...
            pose=str(data.get("pose", cls.pose)),
            yunet=str(data.get("yunet", cls.yunet)),
            sface=str(data.get("sface", cls.sface)),
            emotion=str(data.get("emotion", cls.emotion)),
            active_speaker=str(data.get("active_speaker", cls.active_speaker)),
    ...
    def resolve_pose(self) -> Path:
        return resolve_main_path(self.pose)

    def resolve_yunet(self) -> Path:
        return resolve_main_path(self.yunet)

    def resolve_sface(self) -> Path:
        return resolve_main_path(self.sface)

    def resolve_emotion(self) -> Path:
        return resolve_main_path(self.emotion)

    def resolve_active_speaker(self) -> Path:
        return resolve_main_path(self.active_speaker)
