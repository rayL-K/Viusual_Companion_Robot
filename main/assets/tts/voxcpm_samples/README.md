...
```text
E:\jieya\AI相关\yzylauncher-voxcpm2-tts\python\saved_voices
```

用途是给 VoxCPM 提供参考音色，同时用于检查浏览器音频播放是否正常。它们不是“按当前文本实时生成”的语音，也不再作为语音模型占位项展示。

来源项目为 OpenBMB/VoxCPM，整合包根目录声明 Apache-2.0 许可证。当前生产链路在 ELF2 本地使用 VoxCPM.cpp + VoxCPM1.5 Q4，并默认选择 `soft_girl`；详细版本、校验值与实测见 `main/docs/voxcpm_tts_route.md`。
