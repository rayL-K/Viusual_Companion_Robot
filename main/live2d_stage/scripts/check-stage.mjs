...
assert(stageScript.includes("pauseOfflineAsrForPlayback"), "stage.js 缺少播放语音时暂停识别以避免回声的逻辑。");
...
assert(perceptionScript.includes("emotionOnnxClient.classify(video, faceBox)"), "perception-client.js 未调用 FER+ 情绪适配器。");
...
assert(perceptionScript.includes("blendshape_rule"), "perception-client.js 缺少 FER+ 服务不可用时的 blendshape 规则来源。");
assert(perceptionScript.includes("_landmarkerLoadPromise"), "perception-client.js 缺少视觉模型并发加载复用。");
assert(perceptionScript.includes("generation !== this._generation"), "perception-client.js 缺少摄像头异步启动取消保护。");
assert(emotionOnnxScript.includes("EMOTION_BACKEND_URL"), "emotion-onnx-client.js 缺少 FER+ 服务地址。");
assert(emotionOnnxScript.includes("EMOTION_INPUT_SIZE = 64"), "emotion-onnx-client.js 应按 FER+ 输入尺寸裁剪人脸。");
assert(emotionOnnxScript.includes('method: "POST"'), "emotion-onnx-client.js 未向 FER+ 服务发送请求。");
assert(emotionOnnxScript.includes('source: "ferplus"'), "emotion-onnx-client.js 缺少 FER+ 情绪来源标记。");
assert(!emotionOnnxScript.includes("onnxruntime-web"), "浏览器端不应继续加载已迁移到后端的 ONNX Runtime。");
...
const supportedTtsBackends = ["sherpa_onnx", "voxcpm_hf_space", "voxcpm_project_local", "voxcpm_local", "voxcpm_local_gradio"];
