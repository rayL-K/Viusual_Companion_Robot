...
> **当前运行边界：** 评委主入口 `anima.veyralux.org`（`robot.veyralux.org` 保留为兼容别名）与 ELF2 必须继续运行 V1。V2 只允许在开发电脑和自动化环境中开发验证，暂不上传、安装或切换到开发板。仓库内的 V2 systemd/Cloudflare 文件仅是未来部署草案，激活脚本默认拒绝切换。
