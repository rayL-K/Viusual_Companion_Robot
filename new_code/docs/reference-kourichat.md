...
本文只提炼 `E:\CODE\kourichat` 中可验证的产品与架构思想，不复制其代码。该本地版本使用 **DeepAnima License 1.2（非商业）**，并要求归属；Anima 因此只做 clean-room 设计参考，所有实现均沿用自身接口、数据模型与测试。
...
Anima 应由 `TemporalContextPort` 从持久事件时间线产生：
...
Anima 采用可取消 `TurnCoalescer`，而不是线程定时器：
...
KouriChat 的自动消息包含随机倒计时、安静时段和未回复计数（`src/handlers/autosend.py`）。产品方向可取，但随机选择用户、守护线程和自动重复调度不适合 Anima。

Anima 的 `ProactivePolicyPort` 必须满足：
...
Anima 将其定义为 `ReflectionArtifactPort`：空闲时异步生成、来源可追踪、用户可查看和删除；失败不影响聊天，产物不得被当成用户事实写回 Memory Curator。

## 明确拒绝继承的部分

| KouriChat 现状 | Anima 决策 |
|---|---|
...
7. 许可证扫描确认 Anima 未复制 KouriChat 受限代码。

## 当前状态

本文是设计取舍，不代表上述模块已经全部实现。Anima 当前已具备 generation 取消、SQLite/FTS RAG、事实 provenance、视觉上下文与 AvatarIntent，也已落地匿名 User/Anima 独立数据根及 `Anima.md` 设置；可信账号认证、多 Anima CRUD、主动陪伴策略、TurnCoalescer、TemporalContextPort 和反思产物仍需按路线图逐项实现和验证。
