# Anima v0.0.1 服务器采购与容量规划（2026-07-13）

> **结论先行：采用 API-first 通用服务器，不直接购买双 GPU 服务器。** 第一阶段使用
> Ubuntu CPU 节点承载同源 Gateway、认证、SQLite/备份与观测，文本推理调用 DeepSeek；
> 本地 ASR/TTS/VLM 只在目标模型通过真实延迟和并发验收后启用。需要本地多模态模型时，
> 先按小时租用至少 24 GB VRAM 的 NVIDIA 节点运行 7～14 天，再决定是否购买单 GPU
> 工作站。产品运行时不依赖比赛开发板。
...
| 标签 | 含义 |
| --- | --- |
| **实测** | 本仓库自动化或本机基准产生的结果；不外推为 SoulX、目标服务器或公网性能 |
...
- **未实测**：目标 Linux Server 上的真实 API 组合、SoulX 热/冷启动、真实 VLM、公网 p50/p95、GPU 并发和 30 天常驻。因此下文不能把任何并发数写成“已经达到”。
...
## 2. 负载拆分：通用服务器与 API Provider

通用服务器负责 Gateway、身份、用户/Anima 数据、记忆/RAG、Provider Catalog、备份和可观测性。ASR、Vision、LLM、TTS 由服务器预注册的云 API 或独立本地 sidecar 提供；浏览器只看到允许选择的能力别名，不能提交真实 URL 或密钥。

### 2.1 控制面

1. OIDC 登录、Session/CSRF/Origin 与租户边界；
2. 每个 Anima 的人设、回复策略和不可变 Provider snapshot；
3. 数据保留、导出、删除、配额、备份与恢复；
4. Provider 能力、数据策略、健康、成本、并发和回退规则；
5. 发布、审计与不包含正文/密钥的指标。

### 2.2 实时数据面

1. 浏览器 PCM/JPEG、generation/cancel 和 latest-value 感知时间线；
2. 可取消的 ASR、LLM、TTS 流及有界背压；
3. Live2D 意图、语音能量和播放同步；
4. 上游超时、限流或断流时只按服务器规则回退。

该拆分让初期 CPU 云主机不必购买 GPU；当某个本地模型通过真实 p95、成本和稳定性验收后，再把对应 Adapter 指向隔离 sidecar。任何新服务器都必须通过 candidate health、数据恢复与回切门禁后再接管 Anima 入口。
...
| --- | ---: | --- | ---: | --- |
| A：API-first CPU 单机 | 云活动机或 4–8 vCPU 云主机 | 无 | 从 1 开始实测 | **现在采用** |
...
### 3.1 A 档：云 LLM + 通用 CPU 节点

**适合**：目前的开发、演示和首批 ToC 用户；LLM 调 DeepSeek，CPU 节点负责入口、认证、
数据库、备份、监控和协议转发。本地 sherpa ASR/TTS 按 CPU 实测启用，视觉默认使用已验收的
独立 worker/API；Gateway 的服务可用性不依赖任何实验性本地模型。
...
- CPU 节点本身：约 50～100 个长连接、8～20 个轻量文本/控制活跃会话的规划起点；整条链路仍受上游 API 配额、媒体出网和单机 SQLite 写入模型限制，先从 1 个活跃全模态会话逐级压测。
...
- GPU 0：SoulX 高质量 TTS，先放 2 个 worker；GPU 1：VLM，先放 2 个 7B 4-bit worker；默认低时延 TTS 仍回退到已验收的云 API 或轻量本地 Provider。
...
不通过时，结论应是“保留已验收的低时延云 TTS，SoulX 仅离线生成或不采用”；本地 sherpa 仍可作为隐私/降本选项，而不是再加钱买更大服务器。

---

## 5. 程序、数据、模型、配置和密钥如何落盘

任何新主机都应延续 Anima 的分离原则：

```text
/opt/anima/              # 程序与不可变镜像，只读发布
/var/lib/anima/         # User/Anima 数据库、记忆、审计
/opt/anima/models/       # 可重下的模型资产，不进入用户备份
/etc/anima/              # 非敏感配置与版本化 schema
/run/credentials/anima/  # 启动时注入密钥，不写镜像/仓库
/var/cache/anima/        # TTS/VLM 临时缓存，可清理
```
...
1. **A 档**：通用 Ubuntu CPU 节点 + DeepSeek V4 Flash；低价活动机只适合开发入口，
   不要把 3 Mbps 套餐误认为生产容量。
