...
> 工作站。ELF2 仅是可选边缘/兼容节点，不是公网主链路或容量规划前提。
...
- **未实测**：ELF2 上的完整 Anima、SoulX 热/冷启动、真实 VLM、公网 p50/p95、GPU 并发、30 天常驻、温度与功耗。因此下文不能把任何并发数写成“已经达到”。
...
## 2. 负载拆分：通用服务器、Provider 与可选边缘节点

通用服务器始终负责 Gateway、身份、用户数据、备份和可观测性。ELF2/RK3588 如仍可用，
可以注册为受控边缘节点承担演示或本地媒体推理；它被回收或离线时，公网主链路必须继续工作。
以下板端模式属于兼容选项，不是新用户部署要求：
...
这样可以保留既有 RK3588 资产，又避免未来多用户架构被单板吞吐量锁死。任何新服务器都必须通过 candidate health、数据恢复与回切门禁后再接管 Anima 入口。
...
独立 worker/API；不把任何 ELF2 在线状态作为服务可用性的条件。
